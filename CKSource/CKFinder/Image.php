...
class Image
{
    protected static array $supportedExtensions = ['jpg', 'jpeg', 'gif', 'png'];
     ...
     */
    public function __construct(string $imageData, bool $bmpSupport = false)
    {
     ...
     */
    public function setMemory(int $imageWidth, int $imageHeight, int $imageBits, int $imageChannels): bool
    {
        $MB = 1048576; // number of bytes in 1M
        $K64 = 65536; // number of bytes in 64K
        $TWEAKFACTOR = 2.4; // Or whatever works for you
        $memoryNeeded =
            round(($imageWidth * $imageHeight * $imageBits * $imageChannels / 8 + $K64) * $TWEAKFACTOR) +
            3 * $MB;
        ...
        } elseif ($memoryNeeded + 3 * $MB > $memoryLimitMB) {
     ...
     * @return resource
     */
    public function createFromBmp(string $data)
    {
        ...
        if (24 === $BMP['bits_per_pixel']) {
            while ($Y >= 0) {
                ...
                while ($X < $BMP['width']) {
                    ...
                    imagesetpixel(
                        ...
                        $X++,
                        $Y,
                        ($temp[$offset + 3] << 16) + ($temp[$offset + 2] << 8) + $temp[$offset + 1]
                    );
                }
                --$Y;
            ...
            $COLOR = unpack('n', $VIDE . $IMG[floor($P)]);
            if (0 === ($P * 8) % 8) {
                $COLOR[1] >>= 7;
            } elseif (1 === ($P * 8) % 8) {
     ...
     */
    public static function create(string $data, bool $bmpSupport = false): Image
    {
     ...
     */
    public static function isSupportedExtension(string $extension, bool $bmpSupport = false): bool
    {
    ...
    public static function mimeTypeFromExtension(string $extension): ?string
    {
        ...
        return $extensionMimeTypeMap[$extension] ?? null;
    }
     ...
     */
    public function resize(int $maxWidth, int $maxHeight, int $quality = 80, bool $useHigherFactor = false): static
    {
        ...
        $targetSize = static::calculateAspectRatio(
            $maxWidth,
            $maxHeight,
            $this->width,
            $this->height,
     ...
     */
    public static function calculateAspectRatio(
        int $maxWidth,
        int $maxHeight,
        int $actualWidth,
        int $actualHeight,
        bool $useHigherFactor = false
    ): array {
     ...
     */
    public function fastCopyResampled(
        $dstImage,
        $srcImage,
        int $dstX,
        int $dstY,
     ...
     * Returns image data.
     *
     * @param string|null $format  returned image format MIME type (current image MIME type is used if not set)
     * @param int         $quality Image quality (used for JPG images only)
     *
     * @return string image data
     */
    public function getData(string $format = null, int $quality = 80): string
    {
        $mimeFormat = $format ?: $this->mime;
     ...
     */
    public function getGDImage(): object
    {
        return $this->gdImage;
    }
    ...
    public function rotate(float $degrees, bool|int $bgColor = 0): static
    {
        if ('image/png' === $this->mime) {
            imagesavealpha($this->gdImage, true);
    ...
    #[Pure, ArrayShape(['width' => 'int', 'height' => 'int', 'size' => 'int'])]
