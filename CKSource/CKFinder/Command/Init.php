...
class Init extends CommandAbstract
{
    ...
    public function execute(
        ...
        $lc = str_replace(
            '-',
            '',
            ($config->get('licenseKey') ?: $config->get('LicenseKey')) . str_repeat(' ', 34)
        );
        ...
        if (!empty($resourceTypesNames)) {
            ...
            foreach ($resourceTypesNames as $resourceTypeName) {
                ...
                $resourceTypeObject = [
                    ...
                    'acl' => $aclMask,
                    'maxSize' => $resourceType->getMaxSize()
                        ? min($resourceType->getMaxSize(), $phpMaxSize)
                        : $phpMaxSize,
                ];
                ...
                if ($resourceType->isLazyLoaded()) {
                    ...
                    $resourceTypeObject['hasChildren'] = $resourceTypeBackend->containsDirectories(
                        $resourceType,
                ...
                if ($useProxyCommand) {
                    ...
                    if ($baseUrl) {
                        $resourceTypeObject['url'] = rtrim(
                                Path::combine($baseUrl, $resourceType->getDirectory()),
                                '/'
                            ) . '/';
                    }
                }
