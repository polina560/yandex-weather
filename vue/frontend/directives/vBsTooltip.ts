...
function resolveTrigger(modifiers: DirectiveBinding['modifiers']): Tooltip.Options['trigger'] {
  if (modifiers.manual) return 'manual'

  const trigger: string[] = []

  if (modifiers.click) trigger.push('click')
  if (modifiers.hover) trigger.push('hover')
  if (modifiers.focus) trigger.push('focus')

  if (trigger.length > 0) return trigger.join(' ') as Tooltip.Options['trigger']
...
const vBsTooltip: Directive<HTMLElement> = {
